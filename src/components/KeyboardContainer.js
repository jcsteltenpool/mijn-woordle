...
export default function Keyboard({ onKeyboardClick, disabled, status }) {
    ...
    const keyboardKeys = KEYBOARD_KEYS.map(keyboardKey =>
        <button key={keyboardKey} 
            ...
            <span className="keyboard-key-bg" data-status={status}></span>
            <span className="keyboard-key-content" data-status={status}>
